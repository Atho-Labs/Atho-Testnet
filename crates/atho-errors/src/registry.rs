...
descriptors! {
    ...
    ADDR_INVALID_PREFIX => ("ATHO-ADDR-002", Address, Error, true, false, "Invalid Address Prefix", "The address prefix does not match the expected Atho network format.", "An address was used in the wrong network context.", "Use an address generated for the active network."),
    ...
    NET_INVALID_NETWORK_SELECTION => ("ATHO-NET-001", Network, Error, true, false, "Invalid Network Selection", "The requested Atho network mode is not recognized.", "An invalid network name was provided through the CLI or environment.", "Use atho-testnet in this repository."),
    NET_BLOCK_NETWORK_MISMATCH => ("ATHO-NET-002", Network, Critical, true, true, "Wrong Block Network", "The block belongs to a different Atho network than the active node.", "A block from a different network context was submitted to this node.", "Submit the block to a node running the same network."),
    ...
    TX_LEGACY_LOCK_FORMAT_REJECTED => ("ATHO-TX-015", Transaction, Critical, true, true, "Legacy Lock Format Rejected", "The transaction uses a non-canonical locking script or unlocking script format.", "A legacy/test-form output, alternate lock length, or malformed locking digest was submitted.", "Rebuild the transaction so every spendable output uses the current 32-byte Atho payment digest format."),
    ...
    MEM_DUST_OUTPUT => ("ATHO-MEM-002", Mempool, Error, true, false, "Dust Output Rejected", "The transaction creates an output below Atho's relay dust floor.", "A wallet or sender attempted to create an output smaller than the 100-atom relay minimum.", "Raise every spendable output to at least 100 atoms or combine the value into fees."),
    ...
    DB_PERSISTED_GENESIS_MISMATCH => ("ATHO-DB-005", Database, Fatal, true, false, "Persisted Genesis Mismatch", "The stored genesis state does not match the active network.", "A database from a different network context was reused here.", "Use a network-specific data directory and resync the chainstate."),
    ...
    MINE_REWARD_ADDRESS_REQUIRED => ("ATHO-MINE-003", Mining, Error, true, false, "Mining Reward Address Required", "The node cannot build a production mining template until a valid payout address is configured.", "Mining was started on a non-dev network without ATHO_MINING_REWARD_ADDRESS or atho.conf miningrewardaddress.", "Configure a same-network payout address before requesting or solving block templates."),
