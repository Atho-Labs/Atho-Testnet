...
fn render_request_form(app: &mut DesktopApp, ui: &mut egui::Ui) {
    ui.vertical(|ui| {
        ...
        if app.testnet_faucet_visible() {
            ui.add_space(16.0);
            ui.separator();
            ui.add_space(12.0);
            render_testnet_faucet_panel(app, ui);
        }
    });
}

fn render_testnet_faucet_panel(app: &mut DesktopApp, ui: &mut egui::Ui) {
    widgets::section_header(ui, "Testnet Faucet");
    ui.add_space(8.0);
    widgets::muted_label(
        ui,
        "Need testnet ATHO? Request up to 100 ATHO. Faucet transactions go through the normal mempool and mining path.",
    );
    ui.add_space(10.0);

    egui::Grid::new("testnet_faucet_form")
        .num_columns(2)
        .spacing([12.0, 9.0])
        .min_col_width(120.0)
        .show(ui, |ui| {
            ...
            ui.vertical(|ui| {
                ui.add_sized(
                    [ui.available_width(), 30.0],
                    egui::TextEdit::singleline(&mut app.faucet_manual_address)
                        .hint_text("Leave blank to use the current receive address")
                        .desired_width(f32::INFINITY),
                );
                ui.horizontal_wrapped(|ui| {
                    if ui.button("Use current receive address").clicked() {
                        app.faucet_manual_address.clear();
                    }
                    let resolved = app.faucet_destination_address();
                    if !resolved.is_empty() {
                        widgets::muted_label(ui, &format!("Resolved destination: {resolved}"));
                    }
                });
            });
            ui.end_row();
            ...
            ui.vertical(|ui| {
                ui.horizontal_wrapped(|ui| {
                    ui.add_sized(
                        [110.0, 30.0],
                        egui::TextEdit::singleline(&mut app.faucet_amount)
                            .hint_text("100")
                            .desired_width(110.0),
                    );
                    widgets::muted_label(ui, "ATHO");
                    for preset in [10u64, 25, 50, 100] {
                        if ui.button(format!("{preset} ATHO")).clicked() {
                            app.set_faucet_amount_preset(preset);
                        }
                    }
                });
                widgets::muted_label(
                    ui,
                    "Cooldown: one request per destination/client every 20 blocks.",
                );
            });
            ui.end_row();
        });
    ...
    ui.add_space(10.0);
    let button_enabled = app.faucet_job.is_none();
    if ui
        .add_enabled(
            button_enabled,
            egui::Button::image_and_text(resources::receive_icon(14.0), "Request Testnet ATHO"),
        )
        .clicked()
    {
        if let Err(err) = app.start_testnet_faucet_request() {
            app.faucet_status = format!("Faucet request failed: {err}");
            app.last_error = Some(err);
        }
    }
    ui.add_space(8.0);
    widgets::muted_label(ui, &app.faucet_status);
}
