...
This file is the single-file validation rules rollup for the Atho alpha repo.
It mirrors the consensus validation guide and collects the code-level rules for
transaction validation, UTXO authorization, Falcon witness handling, block
validation, coinbase accounting, monetary policy, chain selection, storage, and
mainnet readiness.
